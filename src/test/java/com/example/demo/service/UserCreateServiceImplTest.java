...
class UserCreateServiceImplTest {
    ...
    class Method1 {
        ...
        void testOK1() throws Exception {
            ...
            // 例外が発生しないこと
            assertThatCode(() -> service.create(OPERATOR, param)).doesNotThrowAnyException();
            ...
            verify(userMapper, times(1)).insert(eq(entity));
            verify(sharedMapper, times(1)).copyFromUser(eq(BASE_ID + "_01"));
            verify(externalApiLogic, times(1)).logOperation(eq("ユーザ登録"), eq(OPERATOR));
        }

        @DisplayName("異常終了：exists：DataAccessException")
        @Test
        void testNG1() throws Exception {
            ...
            // 例外が発生すること
            assertThatThrownBy(() -> service.create(OPERATOR, param)).isInstanceOf(DataAccessException.class);
        ...
        @DisplayName("異常終了：exists：ApplicationException")
        @Test
        void testNG2() throws Exception {
            ...
            // 例外が発生すること
            assertThatThrownBy(() -> service.create(OPERATOR, param)).isInstanceOf(ApplicationException.class);

            // NOTE: 呼び出されるメソッドが想定通りであること
            verify(converter, times(1)).convertToEntity(eq(param));
            verify(departmentMapper, times(1)).existsById(eq(DEPT_ID));
            verify(exceptionCreator, times(1)).create(eq("401"), eq(deptId()));
        }
        ...
        void testNG4() throws Exception {
            ...
            verify(departmentMapper, times(1)).existsById(eq(DEPT_ID));
            verify(userMapper, times(1)).existsById(eq(BASE_ID + "_01"));
            verify(exceptionCreator, times(1)).create(eq("101"), eq(userId()));
        }

        @DisplayName("異常終了：insert：DataAccessException")
        @Test
        ...
        @DisplayName("異常終了：insert：ApplicationException")
        @Test
        void testNG6() throws Exception {
            ...
            verify(userMapper, times(1)).existsById(eq(BASE_ID + "_01"));
            verify(userMapper, times(1)).insert(eq(entity));
            verify(exceptionCreator, times(1)).create(eq("201"), eq(userId()));
        }

        @DisplayName("異常終了：copyUserToUserSummary：DataAccessException")
        @Test
        ...
        @DisplayName("異常終了：copyUserToUserSummary：ApplicationException")
        @Test
        void testNG8() throws Exception {
            ...
            verify(userMapper, times(1)).insert(eq(entity));
            verify(sharedMapper, times(1)).copyFromUser(eq(BASE_ID + "_01"));
            verify(exceptionCreator, times(1)).create(eq("301"), eq(userId()));
        }
